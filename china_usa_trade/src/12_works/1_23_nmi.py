...
# world = pd.merge(world, so_corr[['iso2','iso3']], left_on='iso_a3', right_on='iso3', how='left')
# world.dropna(subset=['iso2'], inplace=True)
# selected_countries = world[world['iso2'].isin(country_codes)]

# %%
# working on this now
comms = set(product_partition.values())
ncomms = len(comms)
colors = px.colors.qualitative.Set1[:ncomms]
colorscale = [[0, color] for color in colors]
...
for comm in comms:
    ...
    # # Filter the world map data for the specified country codes
    selected_countries = world[world['iso2'].isin(country_codes)]
    fig.add_trace(
        go.Choropleth(
            locations=selected_countries['iso3'], 
            z=np.ones(len(selected_countries))*(comm+1),
...
for comm in comms:
    sublist = \
        [code for code in product_partition.keys() if \
            product_partition[code] == comm]
    # community_arr = np.ones(len(sublist))*(comm)
    community_arr = [str(comm)] * len(sublist)
    # tmp_tmp = pd.DataFrame(columns=['iso2', 'community'], data=np.array([sublist, community_arr]))
    tmp_tmp = pd.DataFrame({'iso2': sublist, 'community': community_arr})
    tmp = pd.concat([tmp, tmp_tmp], axis=0)
# merge iso2 and iso3
tmp = pd.merge(tmp, so_corr, on='iso2', how='left')
fig = px.choropleth(
    locations=tmp['iso3'],
    color=tmp['community'],
    color_discrete_sequence=px.colors.qualitative.Set1[:ncomms],
    locationmode='ISO-3',)
# set color legend name to 'community'
fig.update_layout(coloraxis_colorbar=dict(title='community'))
fig.show()
